...
public class PodInfoController {
    ...
    public Map<String, String> hello() {
        return Map.of("message", "Hello from Server B");
    }
    ...
    public Map<String, String> podInfo(@RequestHeader HttpHeaders headers) {
        ...
        result.put("message", "Hello from Server B");
    ...
    /**
     * server-b → (gateway) → server-c /pod-info
     */
    ...
    public Mono<Map<String, Object>> chain(@RequestHeader HttpHeaders headers) {
        ...
        Map<String, Object> serverBInfo = new LinkedHashMap<>();
        serverBInfo.put("message", "Hello from Server B");
        serverBInfo.put("podName", env("POD_NAME"));
        serverBInfo.put("podIp", env("POD_IP"));
        serverBInfo.put("namespace", env("POD_NAMESPACE"));
        serverBInfo.put("nodeName", env("NODE_NAME"));

        return webClient.get()
                .uri("/server-c/chain")
                .headers(h -> {
                    h.addAll(headers);
                    h.set("X-Server-B-Sample", "passed-through-server-b");
                })
                .retrieve()
                ...
                .map(serverCResponse -> {
                    ...
                    result.put("txId", txId != null ? txId : "unknown");
                    result.put("server-b", serverBInfo);
                    result.put("next", serverCResponse);
                    return result;
                });
    }

    private void logHeaders(String endpoint, HttpHeaders headers) {
        log.info("=== [Server-B] /{} Request Headers ===", endpoint);
